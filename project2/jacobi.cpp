...
// void jacobi_method ( mat &A, mat &R)
// A(i,j) = ...
void jacobi_method ( double ** A, double ** R, int n )
{
    ...
    double max_number_iterations = (double) n * (double) n * (double) n; int iterations = 0;
    double max_offdiag = maxoffdiag ( A, &k, &l, n );
    while ( fabs(max_offdiag) > epsilon && (double) iterations < max_number_iterations ) { max_offdiag = maxoffdiag ( A, &k, &l, n );
        rotate ( A, R, k, l, n );
        iterations++;
    }
...
void rotate ( double ** A, double ** R, int k, int l, int n ) {
    ...
    for ( int i = 0; i < n; i++ ) {
        if ( i != k && i != l ) { a_ik = A[i][k];
            a_il = A[i][l];
            A[i][k] = c*a_ik - s*a_il; A[k][i] = A[i][k]; A[i][l] = c*a_il + s*a_ik; A[l][i] = A[i][l];
        }
